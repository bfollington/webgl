...
const CuttingWordsMaterial = shaderMaterial(
  {
    iTime: 0,
...
//  float tm = TIME-2.*length(p);
  ...
  for (float i = 0.; i < 4.; ++i) {
    float v = length(fract(vec2(x - tm*i*xm, fract(y + i*ym)*.125)*16.)*(-2. + -1.*cos(iTime))-1.);
    c = pmin(c, v, 0.0125);
  }
...
vec3 color(vec2 p) {
  ...
  const vec3 lcol1 = HSV2RGB(vec3(0.70, 0.25, 2.0));
  const vec3 lcol2 = HSV2RGB(vec3(0.67, 0.4 , 1.0));
...
extend({ CuttingWordsMaterial })

type CuttingWordsMaterialImpl = {
  iTime: number
...
declare global {
  ...
  namespace JSX {
    interface IntrinsicElements {
      cuttingWordsMaterial: CuttingWordsMaterialImpl
    }
  }
}
...
export function ShaderScene() {
  ...
  return (
    <ScreenQuad ref={boxRef}>
      <cuttingWordsMaterial
        key={CuttingWordsMaterial.key}
        ref={ref}
        iTime={0}
        fftBars={bars}
        scopeX={scopeX}
